...
from cl_layers import ProposedConv
...
class ClusterEncoder(nn.Module):
    ...
    def forward(self, x: Tensor, cluster_index: Tensor, num_nodes: int, num_edges: int):
        for i in range(self.num_layers):
            x, e = self.convs[i](x, cluster_index, num_nodes, num_edges)
            x = self.act(x)
        return x, e # act, act
...
class CL(nn.Module):
    def __init__(self, encoder: ClusterEncoder, proj_dim: int):
    ...
    def forward(self, x: Tensor, cluster_index: Tensor,
                num_nodes: Optional[int] = None, num_edges: Optional[int] = None):
        if num_nodes is None:
            num_nodes = int(cluster_index[0].max()) + 1
        if num_edges is None:
            num_edges = int(cluster_index[1].max()) + 1
        ...
        self_loop_cluster_index = torch.cat([cluster_index, self_loop], 1)
        n, e = self.encoder(x, self_loop_cluster_index, num_nodes, num_edges + num_nodes)
        return n, e[:num_edges]

    def without_selfloop(self, x: Tensor, cluster_index: Tensor, node_mask: Optional[Tensor] = None,
                num_nodes: Optional[int] = None, num_edges: Optional[int] = None):
        if num_nodes is None:
            num_nodes = int(cluster_index[0].max()) + 1
        if num_edges is None:
        ...
        if node_mask is not None:
            ...
            self_loop = torch.stack([node_idx, edge_idx])
            self_loop_cluster_index = torch.cat([cluster_index, self_loop], 1)
            n, e = self.encoder(x, self_loop_cluster_index, num_nodes, num_edges + len(node_idx))
            return n, e[:num_edges]
        else:
            return self.encoder(x, cluster_index, num_nodes, num_edges)
    ...
    def membership_level_loss(self, n: Tensor, e: Tensor, cluster_index: Tensor, tau: float, batch_size: Optional[int] = None, mean: bool = True):
        e_perm = e[torch.randperm(e.size(0))]
        n_perm = n[torch.randperm(n.size(0))]
        if batch_size is None:
            pos = self.f(self.disc_similarity(n[cluster_index[0]], e[cluster_index[1]]), tau)
            neg_n = self.f(self.disc_similarity(n[cluster_index[0]], e_perm[cluster_index[1]]), tau)
            neg_e = self.f(self.disc_similarity(n_perm[cluster_index[0]], e[cluster_index[1]]), tau)

            loss_n = -torch.log(pos / (pos + neg_n))
            loss_e = -torch.log(pos / (pos + neg_e))
        else:
            num_samples = cluster_index.shape[1]
            num_batches = (num_samples - 1) // batch_size + 1
            ...
            for i in range(num_batches):
                ...
                pos = self.f(self.disc_similarity(n[cluster_index[:, mask][0]], e[cluster_index[:, mask][1]]), tau)
                neg_n = self.f(self.disc_similarity(n[cluster_index[:, mask][0]], e_perm[cluster_index[:, mask][1]]), tau)
                neg_e = self.f(self.disc_similarity(n_perm[cluster_index[:, mask][0]], e[cluster_index[:, mask][1]]), tau)
