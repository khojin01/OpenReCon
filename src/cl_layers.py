...
class ProposedConv(MessagePassing):
    ...
    def forward(self, x: Tensor, cluster_index: Tensor, 
        ...
        if num_nodes is None:
            num_nodes = x.shape[0]
        if num_edges is None and cluster_index.numel() > 0:
        ...
        if (cache_norm_n2e is None) or (cache_norm_e2n is None):
            cluster_weight = x.new_ones(num_edges)

            node_idx, edge_idx = cluster_index
            Dn = scatter_add(cluster_weight[cluster_index[1]],
                            cluster_index[0], dim=0, dim_size=num_nodes)
            De = scatter_add(x.new_ones(cluster_index.shape[1]),
                            cluster_index[1], dim=0, dim_size=num_edges)
        ...
        x = self.lin_n2e(x)
        e = self.propagate(cluster_index, x=x, norm=norm_n2e, 
        ...
        x = self.lin_e2n(e)
        n = self.propagate(cluster_index.flip([0]), x=x, norm=norm_e2n, 
