...
def sqrtinvdiag(A):
    """
    Return D^{-1/2} where D = diag(sum_i A_{ij}).
    Works for both scipy sparse and dense numpy arrays.
    """
    if sp.issparse(A):
...
def fix_network(A_p,A_n,labels,eta=0.1):
    ...
    '''
    try:
        G = nx.from_scipy_sparse_array((A_p - A_n).tocoo())
    except AttributeError:
        G = nx.from_scipy_sparse_matrix((A_p - A_n).tocoo())
...
def build_signed_pyg_data(A_pos, A_neg, node_features):
    pos_src, pos_dst = A_pos.nonzero()
    neg_src, neg_dst = A_neg.nonzero()

    edge_index = torch.tensor(
        [list(pos_src) + list(neg_src), list(pos_dst) + list(neg_dst)],
        dtype=torch.long,
    )
    edge_attr = torch.tensor([1] * len(pos_src) + [-1] * len(neg_src),
                            dtype=torch.float)

    x = torch.tensor(node_features, dtype=torch.float)  # shape [N, F]
